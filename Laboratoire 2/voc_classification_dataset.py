...
class VOCClassificationDataset(VOCDetection):
    ...
    def __getitem__(self, index):
        ...
        # Creation d'un vecteur multi-hot pour la classification 21 classes
        multi_hot = torch.zeros(self.nb_classe, dtype=torch.float)

        # ------------------------ Laboratoire 2 - Question 1 - Début de la section à compléter ------------------------

        picture_names = [ target_metadata['annotation']['object'][i]['name'] for i in range(len(target_metadata['annotation']['object']))] # target_metadata['annotation']['object'][0]['name']

        # Trouver la clé du nom dans le dictionnaire
        keys = [self.VOC_CLASSES_2_ID[key] for key in picture_names]
        [multi_hot.__setitem__(key, 1) for key in keys]
